...
class O2Scraper(BaseScraper):
    ...
    async def scrape(self) -> List[ScrapedPlan]:
        ...
        try:
            ...
            if not html:
                logger.warning("O2: No HTML content received")
                return plans
            ...
            logger.info(f"O2: Found {len(plans)} plans")
        except Exception as e:
            logger.error(f"O2 scrape error: {e}")
        return plans
    ...
    def _try_parse_next_data(self, html: str) -> List[ScrapedPlan]:
        ...
        try:
            ...
            logger.debug(f"O2 __NEXT_DATA__ parse failed: {e}")
        return plans
            ...
            try:
                ...
                for offer in offers:
                    ...
                    plans.append(ScrapedPlan(
                        name=name or f"O2 {data_gb}GB" if data_gb else "O2 Plan",
                        price=price,
                        data_gb=data_gb,
                        ...
                        external_id=f"o2_ld_{price}_{data_gb or 'unlimited'}",
                    ))
    ...
    def _try_parse_regex_fallback(self, html: str) -> List[ScrapedPlan]:
        ...
        for price_str in prices:
            ...
            name = f"O2 {data_gb}GB" if data_gb else "O2 Unlimited" if is_unlimited else "O2 Plan"
            plans.append(ScrapedPlan(
                name=name,
                price=price,
                data_gb=data_gb,
                ...
                url=self.base_url,
                is_5g=has_5g,
                external_id=f"o2_regex_{price}_{data_gb or 'unlimited'}",
            ))
        return plans
    ...
    def _extract_plan_from_json(self, p: dict) -> Optional[ScrapedPlan]:
        try:
            ...
            return ScrapedPlan(
                ...
                url=self.base_url,
                is_5g=bool(is_5g),
                external_id=f"o2_json_{price}_{data_gb or 'unlimited'}_{contract}m",
            )
    ...
    def _parse_card_element(self, card) -> Optional[ScrapedPlan]:
        try:
            ...
            if is_unlimited:
                ...
                name = f"O2 {data_gb}GB" if data_gb else "O2 Plan"
            ...
            return ScrapedPlan(
                ...
                url=self.base_url,
                is_5g=is_5g,
                external_id=f"o2_card_{price}_{data_gb or 'unlimited'}_{contract_months}m",
            )
        except Exception:
            return None
    ...
    def _extract_offers_from_jsonld(data) -> list:
        if isinstance(data, list):
            offers = []
            for item in data:
                offers.extend(O2Scraper._extract_offers_from_jsonld(item))
            return offers
        if isinstance(data, dict):
